"""
MR.MIND — Streamlit edition (with voice input + text-to-speech)
...
with tab1:
    ...
    if st.button("🔎 Ask AI"):
        if not query.strip():
            ...
            st.error("Please add your Anthropic API key in the Settings tab first.")
        else:
            with st.spinner("Reasoning over your memories..."):
                ...
                try:
                    answer = call_claude(system_prompt, query)
                    st.markdown(
                    ...
                    if not st.session_state.api_key:
                        ...
                        with st.spinner("Translating..."):
                            try:
                                ...
                                translated = call_claude(system_prompt, m["content"])
...
with tab3:
    st.write(
        "Ask AI and Translate call the Anthropic API using your own key. "
        "The key is kept only in this session's memory (and, optionally, "
        "the `ANTHROPIC_API_KEY` environment variable) — it is never written "
    ...
    st.caption("Get a key at console.anthropic.com → Account → API Keys.")
    st.divider()
    st.write(
